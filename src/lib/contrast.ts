...
export interface FixResult {
  ...
  ratio: number; // achieved WCAG ratio (>= target if achievable)
  achievable: boolean; // false if even the extreme lightness can't reach target
}
...
function zeroedResult(input: Required<ContrastInput>, error: string): ContrastResult {
  return {
    ...
    fixForWcagAA: { hex: "", oklch: "", ratio: 0, achievable: false },
  };
}
...
export function analyze(input: ContrastInput): ContrastResult {
  ...
  if (!fgValid || !bgValid) {
    const which =
      !fgValid && !bgValid ? "foreground and background" : !fgValid ? "foreground" : "background";
    return zeroedResult(
      resolved,
      `Could not parse the ${which} color. Use hex (3/4/6/8), rgb(), hsl(), or oklch().`,
    );
  }
